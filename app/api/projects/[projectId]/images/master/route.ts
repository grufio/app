...
export async function GET(
  ...
  return withProjectRouteAuth(req, projectId, async (_projectReq, context) => {
    ...
    const { data: restoreBase, error: restoreBaseErr } = await context.supabase
      .from("project_images")
    ...
    const cacheKey = `${context.userId}:${bucket}:${img.storage_path}`
    ...
    const { data: signed, error: signedErr } = await context.supabase.storage.from(bucket).createSignedUrl(img.storage_path, SIGNED_URL_TTL_S)
...
export async function DELETE(
  ...
  return withProjectRouteAuth(req, projectId, async (_projectReq, context) => {
    ...
    const { data: imageToDelete, error: fetchErr } = await context.supabase
      .from("project_images")
    ...
    if (!imageToDelete) return jsonError("Image not found", 404, { stage: "not_found" })
    ...
    const { data: targetsRaw, error: targetsErr } = await context.supabase.rpc("collect_project_image_delete_targets", {
    ...
    if (deleteTargets.length > 0) {
      ...
      for (const [bucket, paths] of storageByBucket) {
        ...
        const { error: removeErr } = await context.supabase.storage.from(bucket).remove(paths)
        if (removeErr) {
          storageCleanupFailures.push({ bucket, error: removeErr.message })
        }
      }
      if (storageCleanupFailures.length > 0) {
        console.warn("master-image delete: storage cleanup incomplete", {
    ...
    if (wasActive) {
      const { data: remainingImages } = await context.supabase
        .from("project_images")
