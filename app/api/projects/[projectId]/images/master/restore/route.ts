...
export async function POST(
  ...
  return withProjectRouteAuth(req, projectId, async (_projectReq, context) => {
    const { data: activeImageRow, error: activeImageErr } = await context.supabase
      .from("project_images")
      .select("id,is_locked")
    ...
    const { data: baseMaster, error: baseErr } = await context.supabase
      .from("project_images")
    ...
    const { data: workspaceRow, error: workspaceErr } = await context.supabase
    ...
    const { error: rpcErr } = await context.supabase.rpc("set_active_master_with_state", {
