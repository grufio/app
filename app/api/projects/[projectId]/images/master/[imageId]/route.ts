...
export async function DELETE(
  ...
  return withProjectRouteAuth(req, projectId, async (_projectReq, context) => {
    ...
    if (!imageToDelete) {
      const targetLookup = await getEditorTargetImageRow(context.supabase, context.projectId)
    ...
    const { error: deleteErr, count } = await context.supabase
      .from("project_images")
      .delete({ count: "exact" })
      .eq("id", imageId)
      .eq("project_id", context.projectId)
    ...
    if (wasActive) {
      ...
      if (promote) {
        ...
        const { error: promoteErr } = await context.supabase
          .from("project_images")
