...
export function deriveEditorSourceSnapshot(args: {
  ...
  /** Trace-free raster source. **This is the workflow-source identity
   * post PR #109**: canvas display, image-state load/save target, and
   * filter-source resolver all key off this. The trace-aware
   * `filterDisplayImage` is used elsewhere only for the SVG overlay. */
