...
export interface Question {
  ...
  answer: string;
  /** 5 options (1 correct + 4 distractors), already shuffled. */
  options: string[];
}
 ...
 *
 * `rng` makes the result deterministic for a given seed (used in tests).
 */
export function buildQuestion(
  ...
  return { item, direction, prompt, answer, options };
}
