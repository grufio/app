...
describe("lineArtImageAndActivate validation contract", () => {
  ...
  it("rejects line_thickness > 10", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 11, blur_amount: 3, smoothness: 0.6, num_colors: 8 },
    })
  ...
  it("rejects line_thickness < 1", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 0, blur_amount: 3, smoothness: 0.6, num_colors: 8 },
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.stage).toBe("validation")
  })
  ...
  it("rejects blur_amount > 20", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 2, blur_amount: 21, smoothness: 0.6, num_colors: 8 },
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.stage).toBe("validation")
  })
  ...
  it("rejects smoothness > 1", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 2, blur_amount: 3, smoothness: 1.5, num_colors: 8 },
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.stage).toBe("validation")
  })
  ...
  it("rejects num_colors < 2", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 2, blur_amount: 3, smoothness: 0.6, num_colors: 1 },
    })
    expect(result.ok).toBe(false)
    if (!result.ok) expect(result.stage).toBe("validation")
  })
  ...
  it("accepts boundary values and continues to source lookup", async () => {
    const result = await lineArtImageAndActivate({
      ...
      projectId,
      sourceImageId,
      params: { line_thickness: 1, blur_amount: 0, smoothness: 0, num_colors: 2 },
    })
    expect(result.ok).toBe(false)
