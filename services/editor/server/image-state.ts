 ...
 * - State is anchored at the project's working_copy.id (PR #257).
...
export async function getImageStateForEditor(
  ...
  if ("error" in anchor) return { imageState: null, error: anchor.error, unsupported: false }
  if ("notFound" in anchor) return { imageState: null, error: null, unsupported: false }

  const { row: st, error: stErr, unsupported } = await loadBoundImageState(supabase, projectId, anchor.id)
