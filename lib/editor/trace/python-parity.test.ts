...
describe("Python parity: TS trace schema defaults vs Pydantic", () => {
  ...
  it("extracts the expected fields (regression guard for parser)", () => {
    // The parser only pulls fields with a scalar-typed default (union-typed
    // `palette_oklab: list | None` is skipped), so the stable markers are the
    // scalar dials + the server-computed defaults.
    expect(Object.keys(extractPydanticDefaults("LinerateRequest"))).toEqual(
