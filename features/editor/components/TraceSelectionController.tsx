...
export function TraceSelectionController({ workingImageUrl, open, onClose, onSelect }: Props) {
  ...
  return (
    <Dialog open={open} onOpenChange={(isOpen) => !isOpen && handleClose()}>
      {/* Fullscreen on every viewport (desktop matches mobile): a scrollable
          tile grid between a sticky header and a sticky Cancel/Select footer.
          Tiles forced to 2 columns + tighter gap so they stay small. */}
