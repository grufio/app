...
- **`components/editor/` (UI & Composition)**
  ...
  - **`artboard-panel.tsx`**: Artboard controls (unit/dpi/size) + persistence
  - **`image-panel.tsx`**: Image size controls (always uses artboard unit + dpi)
  - **`index.ts`**: module barrel exports
- **`lib/editor/` (State, Commands, Model)**
  - **Types/Model**: `canvas-model.ts` (viewport/world math)
  - **Units/Conversion**: `units.ts` (mm/cm/pt/px ⇄ px)
