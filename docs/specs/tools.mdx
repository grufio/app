...
- **`components/editor/` (UI & Composition)**
  - **`EditorShell.tsx`**: Layout (links Tools, rechts Canvas, oben Status)
  - **`ToolSidebar.tsx`**: nur UI (Icons, active state)
  - **`CanvasStage.tsx`**: Konva Stage/Layer Renderer
  - **`nodes/`**: `ImageNode.tsx`, später `GridLayer.tsx`, `OverlayNumbers.tsx`, …
  - **`tools/`**: Tool-Controller/Hooks (Hand/Select/Crop) – reine Input-Logik
- **`lib/editor/` (State, Commands, Model)**
  - **Types/Model**: `viewport`, `scene`, `selection`, `tool`
  - **Commands**: `zoomToPoint`, `panBy`, `fitToView`, `rotateSelection`, `setTool`
