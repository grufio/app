...
- **`features/editor/components/` (UI & Composition)**
...
- **`lib/editor/` (State, Commands, Model)**
  ...
  - **Units/Conversion**: `units.ts` (mm/cm/pt/px ⇄ µpx, display formatting: up to 4dp, trim zeros)
  - **Hooks**: `use-display-size.ts` (the single authoritative display-size source — seed/save µpx image working state, re-seed on master transition; no save on unit change)
