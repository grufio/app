...
| `lib/supabase/project-images.ts#activateMasterWithState` | active image switch RPC wrapper | yes | `409/lock_conflict/image_locked` | data-access | done | `manual:code-review` |
...
| `app/api/projects/[projectId]/images/master/[imageId]/route.ts#DELETE` | delete image | yes | `409/lock_conflict/image_locked`, `409/active_conflict/image_not_active` | editor-api | done | `manual:docs/runbooks/optimization-sprint-2-rollout.md#post-deploy-verification-checklist` |
...
| `app/api/projects/[projectId]/image-state/route.ts#POST` | persist transform state (active image bound) | yes | `409/active_image_mismatch`, `409/lock_conflict/image_locked` | editor-api | done | `test:lib/api/route-lock-guards.test.ts` |
...
- Additional invariant: `image-state` writes must include `image_id` and it must equal the server-resolved active image id at write time.
- Additional invariant: restore/delete flows must never operate on stale image references; stale targets return explicit conflict stages.
