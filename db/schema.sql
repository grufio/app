...
-- =========================================================
-- BEGIN db/057_fix_filter_renumber_no_negative.sql
-- =========================================================
...
-- =========================================================
-- BEGIN db/059_delete_project_bypass_master_guard.sql
-- =========================================================
...
$$;
-- =========================================================
-- END db/059_delete_project_bypass_master_guard.sql
-- =========================================================


-- =========================================================
-- BEGIN db/060_advisory_lock_active_state_rpcs.sql
-- =========================================================
-- Concurrency: extend project advisory lock to the active-state RPCs.
--
-- Filter-chain RPCs (append/remove/reorder) already serialize per-project
-- via `pg_advisory_xact_lock(hashtext(project_id))`. The "make this image
-- the active master" path
--   set_active_image → set_active_master_image
--                    → set_active_master_latest
--                    → set_active_master_with_state
-- ran without that lock, so:
--
--   - Master upload + concurrent filter-apply could race
--   - Crop + concurrent filter-apply could race
--   - Two browser tabs editing the same project: last-write-wins on
--     partial state
--
-- Postgres advisory locks are reentrant within the same transaction, so
-- nested calls (set_active_master_with_state → set_active_image)
-- re-acquire safely without deadlock.
...
notify pgrst, 'reload schema';
-- =========================================================
-- END db/060_advisory_lock_active_state_rpcs.sql
-- =========================================================
